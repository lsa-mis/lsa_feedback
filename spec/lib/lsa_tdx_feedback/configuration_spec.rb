...
RSpec.describe LsaTdxFeedback::Configuration do
  ...
  describe '#validate!' do
    ...
    context 'when required values are missing' do
      ...
      it 'raises an error when oauth_url is missing' do
        all_required_values.each { |key, value| config.send("#{key}=", value) }
        config.oauth_url = nil
        expect { config.validate! }.to raise_error(LsaTdxFeedback::Error, 'oauth_url is required')
      end

      it 'raises an error when client_id is missing' do
        all_required_values.each { |key, value| config.send("#{key}=", value) }
        config.client_id = nil
        expect { config.validate! }.to raise_error(LsaTdxFeedback::Error, 'client_id is required')
      end

      it 'raises an error when service_offering_id is missing' do
        all_required_values.each { |key, value| config.send("#{key}=", value) }
        config.service_offering_id = nil
        expect { config.validate! }.to raise_error(LsaTdxFeedback::Error, 'service_offering_id is required')
      end

      it 'raises an error when account_id is missing' do
        all_required_values.each { |key, value| config.send("#{key}=", value) }
        config.account_id = nil
        expect { config.validate! }.to raise_error(LsaTdxFeedback::Error, 'account_id is required')
      end
    end
  end
end
